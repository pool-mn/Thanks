...
uint256 hashGenesisBlock("0x00000308886b5016c6e1793c4c4360b6dbddb48eaa6f06ac8126df12bbfc4085"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Thanks: starting difficulty is 1 / 2^12
...
CScript CHARITY_SCRIPT2;

CScript CHARITY_SCRIPT3;

CScript CHARITY_SCRIPT4;

CScript CHARITY_SCRIPT5;

CScript CHARITY_SCRIPT6;

CScript CHARITY_SCRIPT7;

CScript CHARITY_SCRIPT8;

CScript CHARITY_SCRIPT9;

CScript CHARITY_SCRIPT10;



const string strMessageMagic = "Thanks Signed Message:\n";
...
{
    // Thanks: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 60; // Thanks: 1 day
static const int64 nTargetSpacing =  60; // Thanks: 150 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Thanks: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, Thanks - DarkGravity v2, written by Evan Duffield - evan@Thanks.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
            ...
            else if (pindexLast->nHeight+1 >= 2) { DiffMode = 2; }         
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Thanks: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Thanks: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000888d403167b1a6fd527a61c5f0e1fd8fb736940850e18ac1a0a7b193ae0");
    }
...
bool InitBlockIndex() {
 ...
 CHARITY_SCRIPT2 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS2) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT3 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS3) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT4 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS4) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT5 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS5) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT6 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS6) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT7 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS7) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT8 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS8) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT9 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS9) << OP_EQUALVERIFY << OP_CHECKSIG;
 CHARITY_SCRIPT10 << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS10) << OP_EQUALVERIFY << OP_CHECKSIG;
    ...
    if (!fReindex) {
         ...
         // Genesis block        
        const char* pszTimestamp = "CFROXOONEWTENCHARYOYO";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401251143;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2104956;
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6491d3d4a90198e9d9a8f458eaa000a9e992bdf7add2217ac62b2c6af22f0191"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // Thanks: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    txNew.vout.resize(11);
    txNew.vout[0].scriptPubKey = CHARITY_SCRIPT;
    txNew.vout[1].scriptPubKey = CHARITY_SCRIPT2;

    txNew.vout[2].scriptPubKey = CHARITY_SCRIPT3;

    txNew.vout[3].scriptPubKey = CHARITY_SCRIPT4;

    txNew.vout[4].scriptPubKey = CHARITY_SCRIPT5;

    txNew.vout[5].scriptPubKey = CHARITY_SCRIPT6;

    txNew.vout[6].scriptPubKey = CHARITY_SCRIPT7;

    txNew.vout[7].scriptPubKey = CHARITY_SCRIPT8;

    txNew.vout[8].scriptPubKey = CHARITY_SCRIPT9;

    txNew.vout[9].scriptPubKey = CHARITY_SCRIPT10;


    txNew.vout[10].scriptPubKey = scriptPubKeyIn;
	...
	int64 charityAmount = reward * 0.5 / 100;

	int64 charityAmountT = reward * 5 / 100;
        ...
        pblock->vtx[0].vout[2].nValue = charityAmount;


        pblock->vtx[0].vout[3].nValue = charityAmount;


        pblock->vtx[0].vout[4].nValue = charityAmount;


        pblock->vtx[0].vout[5].nValue = charityAmount;


        pblock->vtx[0].vout[6].nValue = charityAmount;


        pblock->vtx[0].vout[7].nValue = charityAmount;


        pblock->vtx[0].vout[8].nValue = charityAmount;


        pblock->vtx[0].vout[9].nValue = charityAmount;


	pblock->vtx[0].vout[10].nValue = reward - charityAmountT + nFees;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ThanksMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ThanksMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThanksMiner(CWallet *pwallet)
{
    printf("ThanksMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Thanks-miner");
    ...
    try { loop {
        ...
        printf("Running ThanksMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ThanksMiner, pwallet));
}
